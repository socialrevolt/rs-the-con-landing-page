 ...
 *     2. Concatenates all the files and generates script.min.js
 *     3. Renames the JS file with suffix .min.js
 *     4. Uglifes/Minifies the JS file and generates custom.min.js
 */
gulp.task( 'scripts', () => {
	return gulp
		.src([config.jsVendorSRC, config.jsCustomSRC]) // Only run on changed files.
		.pipe( plumber( errorHandler ) )
		.pipe(
			babel({
				presets: [
...
gulp.task(
	'default',
	gulp.series('clearCache', 'styles', 'scripts', 'images', browsersync, () => {
		gulp.watch( config.watchPhp, reload ); // Reload on PHP file changes.
		gulp.watch( config.watchStyles, gulp.parallel( 'styles', reload  ) ); // Reload on SCSS file changes.
		gulp.watch( config.watchScripts, gulp.parallel( 'scripts', reload  ) ); // Reload on script file changes.
		gulp.watch( config.imgSRC, gulp.parallel( 'images', reload ) ); // Reload on customJS file changes.
	})
);
